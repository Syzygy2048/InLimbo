...
Altimeter::Altimeter(glm::mat4& matrix, Shader* _shader) : SceneObject(matrix), shader(_shader){
	textureID = 11;
	...
	if( scene->HasMeshes() ) {
		for(unsigned int i = 0; i < scene->mNumMeshes; ++i){
			...
			aiString texPath = aiString("Models/hoehenmesser.png");
...
void Altimeter::update(double deltaT, int key, float speed){

	int width, height;
	glfwGetWindowSize(&width, &height);

	glm::vec3 p0  = glm::vec3(0.0f, 0.2f, 1.2f);
	glm::vec3 pref = glm::vec3(0.0f,0.0f,-1.0f);
	glm::vec3 v = glm::vec3(0.0f,1.0f,0.0f);

	viewMatrix = glm::lookAt(p0, pref, v);
	projectionMatrix = glm::perspective(25.0f, (float)width / (float) height, 0.1f, 200000000.0f);
	//projectionMatrix = glm::ortho(0.0f,(float)width, 0.0f, (float)height,  -100.0f, 100.0f);
	//projectionMatrix = glm::ortho(0.0f, 300.0f, 400.0f,0.0f, -1.0f, 1.0f);
}
...
btRigidBody* Altimeter::getRigid() {
